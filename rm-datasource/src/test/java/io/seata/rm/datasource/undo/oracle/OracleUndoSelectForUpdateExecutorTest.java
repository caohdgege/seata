 ...
 * @author chd
 */
public class OracleUndoSelectForUpdateExecutorTest extends BaseExecutorTest {

    private static OracleUndoSelectForUpdateExecutor executor;
    private static TableMeta tableMeta;

    @BeforeAll
    public static void init(){
        TableMeta tableMeta = OracleUndoSelectForUpdateExecutorTest.tableMeta = Mockito.mock(TableMeta.class);
        ...
        executor = new OracleUndoSelectForUpdateExecutor(sqlUndoLog);
    }
    ...
    public void buildUndoSQL() {
        ...
        Assertions.assertEquals("SELECT ? FROM dual", sql);

        // multi primary key
        Mockito.when(tableMeta.getPrimaryKeyOnlyName()).thenReturn(Arrays.asList(new String[]{"id", "age"}));
        sql = executor.buildUndoSQL();
        Assertions.assertNotNull(sql);
        Assertions.assertEquals("SELECT ?, ? FROM dual", sql);
    }
